...
#
# Vendor-first: when ../../vendor/oh-my-bash exists this installs by
...
#
# The theme ships inside the repo root (../../bash/theme) so the
...
THEME_SRC="${SCRIPT_DIR}/../../bash/theme/${THEME_NAME}/${THEME_NAME}.theme.bash"
...
ensure_theme_exists() {
  if [[ ! -f "${THEME_SRC}" ]]; then
    ...
    echo "The theme ships at the repo root — run this script from tools/ohmybash/ inside"
    echo "the dev-env repo, which contains bash/theme/${THEME_NAME}/."
    exit 1
  fi
}
