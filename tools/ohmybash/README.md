...
MesloLGS Nerd Font is vendored at `../../vendor/fonts/meslolgs-nf` (four faces:
...
| Client | Installer |
|---|---|
| Windows (Windows Terminal, PuTTY) | `../../windows/install-nerd-font.ps1` — per-user, no admin |
| Linux / macOS workstation, via the whole package | `../../install.d/30-fonts.sh`, run by `install.sh (repo root)` |
...
```
==> Source: VENDORED tree at /path/to/vendor/fonts/meslolgs-nf (offline, no network)
```
...
`install.d/30-fonts.sh` skips itself entirely on a host with no fontconfig, which
...
Step 2 is **vendor-first**. When `../../vendor/oh-my-bash` exists — it does in
...
```
  Oh My Bash source: vendored tree (offline) — /path/to/repo/vendor/oh-my-bash
  - Source: VENDORED tree at /path/to/repo/vendor/oh-my-bash (offline, no network)
```
...
`../../vendor/blesh`. Nothing is compiled, so `make` is never required.
...
whole, use `install.sh (repo root)`, which runs `install.d/20-bash.sh`. That hook
...
Edit `../../bash/theme/devops-powerline/devops-powerline.theme.bash` locally, then re-run `deploy.sh`. It overwrites only the theme file and updates `OSH_THEME` as needed.
...
Everything `deploy.sh` installs — the theme and both vendored trees — lives at
the repo root, which is what gets bundled for an air-gapped host:

```
dev-env/ (repo root — becomes ~/.dotfiles on a target)
├── bash/theme/
│   └── devops-powerline/
...
The dependency runs one way: `deploy.sh` (in tools/) reaches into the repo root,
and nothing at the root reaches back into tools/. That is what lets the root be
extracted on its own as `~/.dotfiles` and still work.
