# dev-env — a portable, offline-first DevOps environment

One repo to pull down wherever you work: tmux, Oh My Bash with a powerline
prompt, git shortcut functions, Nerd Fonts, and (in `tools/`) the VS Code
air-gap kit. Built for air-gapped machines: no internet access needed, and
nothing beyond a `tmux` and `fzf` binary plus the OS base repos. Everything
else ships inside this repo — tmux plugins, the Oh My Bash framework, ble.sh,
fonts. `install.sh` never touches a network, a package manager, or git.
...
```bash
git clone https://github.com/djlongy/dev-env ~/.dotfiles && ~/.dotfiles/install.sh
```
...
installer machinery, not something to browse day-to-day. From any clone:

```bash
git archive --format=tar.gz -o dev-env.tar.gz HEAD
scp dev-env.tar.gz user@host:
ssh user@host 'mkdir -p ~/.dotfiles && tar xzf dev-env.tar.gz -C ~/.dotfiles && ~/.dotfiles/install.sh'
```

(`git archive` excludes `.git`, keeps a clean tree, and adds no macOS xattr
noise. The package is self-contained: the prompt theme, the git shortcut
functions and every vendored tree ship inside it, so `~/.dotfiles` needs
nothing beside it.)
